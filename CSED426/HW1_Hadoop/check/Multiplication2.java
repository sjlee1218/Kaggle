...
public class Multiplication2 {
	...
	public static class Matrix2_Mapper extends Mapper<Text, Text, Text, Text> {
		...
		ArrayList<String> matname_array=new ArrayList<String>();
		...
		public void map(Text matrix, Text entry, Context context) throws IOException, InterruptedException {
			...
			int find=-1;
			find=matname_array.lastIndexOf(matName);// 행렬 이름 찾음
			if (find>=0){ // 처음 찾은 이름이 처음에 들어가야함

			}
			else{ // 못찾으면 넣어줘야함.
				matname_array.add(matName);
			}// matname_array[0]이 첫 행렬 이름 matname_array[1] 두번째 행렬 이름 [2]가 c
			String first_mat_name=matname_array.get(0);

			if (matname_array.size()==1 && matName.equals(first_mat_name)){
			...
			if (matname_array.size()==2){
				String second_mat_name=matname_array.get(1);
				if (matName.equals(first_mat_name)){
					for (int k=0;k<n_second_cols;k++){
						Text key= new Text(words[0]);// key=i,k
						//Text out_value=new Text(matName+",first"+","+words[0]+","+Integer.toString(k)+","+words[2]);
						Text out_value=new Text(matName+",first"+","+line+","+String.valueOf(k));
				...
				//			else if (matName==matname_array.get(1)){
				else if (matName.equals(second_mat_name)){
			...
			if (matname_array.size()==3){
				String second_mat_name=matname_array.get(1);
				String third_mat_name=matname_array.get(2);
				if (matName.equals(first_mat_name)){
					for (int k=0;k<n_second_cols;k++){
						Text key= new Text(words[0]);// key=i,k
						//Text out_value=new Text(matName+",first"+","+words[0]+","+Integer.toString(k)+","+words[2]);
